...
<html lang="<?php $this->load->view('data/lang') ?>">
    ...
    <body>
                        ...
                        <div class="row">
                            <div class="col-md-6 col-lg-6 col-xl-12">
                                <form id="demo-form2"  autocomplete="off">
                                    <label for="id-date-range-picker-1">Fecha de ingreso</label>
                                    <div class="input-group my-group"> 
                                        <input class="form-control" placeholder="Ingresar rango" value="" type="text" name="fecha" id="fecha" required="" />
                                        <select id="idproducto" data-plugin-selectTwo class="form-control" required="" >
                                            <option valor="0" idproducto="" cantidad="0" restante="0" value="">Seleccionar Producto</option>
                                            <?php foreach ($idproducto as $x): ?>
                                                <option value="<?= $x->idproducto ?>"><?= $x->nombre ?></option>
                                            <?php endforeach; ?>
                                        </select> 
                                        <span class="input-group-btn">
                                            <button class="btn btn-primary my-group-button" type="submit">Filtrar <span class="fa fa-filter"></span></button>
                                        </span>
                                    </div>
                                </form>
                            </div>
                            ...
                            <div class="col-md-12 col-lg-5 col-xl-5">
                                <section class="panel">
                                    <div class="panel-body" id="">
                                        <table class="table table-bordered" id="" data-swf-path="assets/vendor/jquery-datatables/extras/TableTools/swf/copy_csv_xls_pdf.swf">
                                            <thead id="">
                                                <tr>
                                            ...
                                            <tbody id=""> 
                                                ...
                                                <?php if (count($idproductos) == 0) { ?>
                                                    <tr><td id="" colspan="4" align="center">Ningún dato</td></tr>
                                                <?php } ?>
                                            </tbody>
                                            <tfoot>
                                                <tr>
        ...
        <script>
            $(function () {
                var column = ['N°', 'Ingreso', 'Caducidad', 'Tipo', 'Cantidad', 'Stock'];
                ...
                var tabla = {
                    ...
                    dibujar: function () {
                        ...
                        for (var i in json) {
                            ...
                            html += '<td align="center" style="background-color:#fcfcfc"><span class="fa fa-calendar-o"></span> ' + ins.fechaingreso + '</td>';
                            html += '<td align="center" style="background-color:#fcfcfc"><span class="fa fa-calendar-o"></span> ' + ins.fechaexpiracion + '</td>';
                            html += '<td align="center" style="background-color:#fcfcfc">' + ins.tipop.toUpperCase() + '</td>';
                ...
                var fechai = '';
                var fechaf = '';
                $("#demo-form2").bind("submit", function (e) {
                    // Capturamnos el boton de envío
                    e.preventDefault();
                    var idproducto = $("#idproducto").val();
                    var data = $(this).serialize();
                    $.ajax({
                        'url': '<?= base_url() ?>inventario/verlistar',
                        'type': 'POST',
                        'data': {fi: fechai, ff: fechaf, idproducto: idproducto},
                        'dataType': 'json',
                        'timeout': 15000,
                        beforeSend: function () {
                            $('#tbody tr').remove();
