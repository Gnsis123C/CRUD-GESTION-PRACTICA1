    ...
    <body>
        ...
        <script>
            $(function () {
                ...
                //FECHA NACIMIENTO
                var imgU = '<?= base_url() ?>static/img/compra.jpg';
                ...
                var url = '<?= base_url() ?>compra/listar';
                var urlelim = '<?= base_url() ?>compra/eliminar';
                var validarcedula = '<?= base_url() ?>compra/validar';
                var urlForm = '<?= base_url() ?>compra/crear';
                var dataTable;
                var id_text = '<?= $id ?>';
                dataTable = $.tablaAjax({
                    fixedHeader: false,
                    ...
                    id: id_text,
                    thead: ['Nª Factura', 'Proveedor', 'Fecha', 'Ir al Detalle', 'Devolver'],
                    ajax: {
                        url: url,
                        urlelim: urlelim,
                        data: {"action": "Jsons"},
                        resp: [
                            {
                                "data": "idcompra", "render": function (d, t, f) {
                                    return d
                                }
                            }, //1
                            {
                                "data": function (d, t, f) {
                                    return d.nombre.toUpperCase() + ' ' + d.apellido.toUpperCase()
                                }
                            },
                            {"data": "fecha"},
                            {
                                "data": "idcompra", "render": function (data, type, full) {
                                    return '<a class="btn btn-success btn-xs" href="<?= base_url() ?>compra/detalle?id=' + data + '">Ir al detalle</a>';
                                },
                                "visible": true
                            },
                            {
                                "data": function (data, type, full) {
                                    return (data.devuelto == '0') ? '<a class="btn btn-default btn-xs" href="<?= base_url() ?>devolver?id=' + data.idcompra + '">Devolver</a>' : '<div class="btn-group" role="group"><a class="btn btn-warning btn-xs" href="<?= base_url() ?>devolver/vista?id=' + data.idcompra + '">Ver devolución</a><button class="btn btn-default btn-xs eliminardevol" value="' + data.idcompra + '" title="Eliminar">X</button></div>';
                                },
                                "visible": true
                            }
                        ]
                ...
                $('#tabla').DataTable().on("draw", function () {
                    $('.eliminardevol').on('click', function () {
                    var btn = $(this);
                    var id = $(this).val();
                    $.ajax({
                        url: '<?= base_url() ?>devolver/eliminardevol',
                        type: 'POST',
                        data: {id: id},
                        dataType: 'JSON',
                        beforeSend: function () {
                            $('.eliminardevol').attr('disabled', true);
                            btn.html('<i class="fa fa-refresh fa-pulse"></i>');
                        }
                    }).done(function (data) {
                        dataTable.ajax.reload(function () {
                            $('.eliminardevol').attr('disabled', false);
                            btn.html('X');
                            return;
                        });
                    }).fail(function () {
